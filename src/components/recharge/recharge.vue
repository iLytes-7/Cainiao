    ...
    <div class="method">
      ...
      <div class="transferAmount">
        ...
        <div class="amountList">
          <div v-for="(item,index) in amountData" :key="item" :class="{amountActive:item==amountcurr}" @click="selectAmount(item)">
            {{item}}元
          </div>
        </div>
      </div>
      <van-button class="btn" color="#FF6D44">立即存款</van-button>
    </div>
  </div>
</template>
