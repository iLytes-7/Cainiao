<template>
  <div class="recharge">
    <div class="method">
      <p>充值方式</p>
      <ul>
        <li @click="selectAutoMethod(index,item)" v-for="(item,index) in totalMethodList" :key="index"
        ...
        <van-radio-group v-model="methodRadio">
          <van-cell-group>
            <van-cell clickable v-for="(item,index) in tempList.list" :key="index"
                      @click="selectMethodRadio(index,item)">
    ...
    <div class="hiddenForm" style="visibility:hidden">
      <form id="simpleApiForm" :action="thirdPartyRequestForm.url" name="simpleApiForm" method="post" target="_blank" />
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectAutoMethod(index, data) {
        this.autoCurr = index
        this.methodRadio = 0
        this.tempList = this.totalMethodList[index]
        this.amount = data.list[0].minDeposit
      },
      change(val) {
      ...
      selectMethodRadio(item, data) {
        this.methodRadio = item;
        this.amount = data.minDeposit
      },
        ...
        this.$dialog.confirm({
          ...
          thirdPartyDepositRequest(data).then(response => {
            ...
            if (response.result.type_text === 'form') {
              this.thirdPartyRequestForm.url = url
              this.thirdPartyRequestForm.params = data
              this.$nextTick(() => {
                var params = this.thirdPartyRequestForm.params
                var form = document.getElementById('simpleApiForm')
                ...
                document.forms['simpleApiForm'].submit()
              })
            }
      ...
      handleCharge() {
        ...
        if (this.autoCurr < length) {
          if (Number(this.amount) > this.tempList.list[this.methodRadio].maxDeposit || Number(this.amount) <
            ...
            this.$toast('当前支付方式单笔限额¥'+this.tempList.list[this.methodRadio].minDeposit +'-¥'+this.tempList.list[this.methodRadio].maxDeposit);
            return
          }
          let data = {
            api_key: "ea443b05c7067089bd2716f47257ee73",
